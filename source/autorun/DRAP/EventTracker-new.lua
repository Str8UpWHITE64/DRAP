...
local function ensure_solid_log_manager()
    ...
    if not success_list_field then
        ...
        if not success_list_field then
            ...
            log("Found field '" .. SUCCESS_LIST_FIELD_NAME .. "' on SolidLogManager.")
        end
    end
...
local function handle_scq_updates()
    ...
    if count > last_scq_list_count then
        for i = last_scq_list_count, count - 1 do
            local raw = get_scq_item_string(list, i) or "(read error)"
            log(string.format("NEW SCQ success [%d]: %s", i, raw))

            local desc = TRACKED_SCQ_IDS[raw]
            if desc and M.on_tracked_check then
                log(string.format("Tracked SCQ completed: %s (%s)", desc, raw))
                pcall(M.on_tracked_check, "SCQ", raw, nil, desc)
            end
        end
        last_scq_list_count = count
    end
    ...
    if ensure_game_manager() then
        ...
        if ok_no then
            ...
            elseif event_no ~= last_event_no then
                ...
                -- Unified tracked check hook
                local tracked_desc = TRACKED_EVENT_IDS[event_no]
                if tracked_desc and M.on_tracked_check then
                    log(string.format(
                        "Tracked Event reached: %s (%d, enum=%s)",
                        tracked_desc, event_no, new_name
                    ))
                    pcall(M.on_tracked_check, "EventNo", event_no, new_name, tracked_desc)
                end
